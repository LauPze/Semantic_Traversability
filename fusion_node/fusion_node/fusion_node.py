...
#        self.calib_transform_path = self.get_parameter('calib_transform_path').get_parameter_value().string_value
    ...
    def odom_callback(self, msg):
        ...
        if not self.origin_set:
            self.origin_pos = np.array([x, y, z])
            self.origin_set = True
            self.get_logger().info(f"Origine odom définie: {self.origin_pos}")

        relative_pos = np.array([x, y, z]) - self.origin_pos
    ...
    def transform_points_to_odom(self, points):
        if self.latest_odom is None:
            self.get_logger().warn("Pas encore d'odom, les points resteront locaux.")
            return points  # Pas de transformation si pas d'odom

        q = self.latest_odom['orientation']
        t = self.latest_odom['position']
        ...
        R = quat_to_rot_matrix(q)  
        # Appliquer une rotation de 180° autour de Z (axe vertical)
...
#        points = points[valid]
        ...
        labels[valid_label] = labels_img[v[valid_label].astype(int), u[valid_label].astype(int)]
#        labels[valid_proj] = labels_img[v[valid_proj], u[valid_proj]]

#        points = points[valid_proj]
#        labels = labels[valid_proj]
        
        points_global = self.transform_points_to_odom(points)
        ...
        cloud_data_np['x'] = points_global[:, 0]
        cloud_data_np['y'] = points_global[:, 1]
        cloud_data_np['z'] = points_global[:, 2]
